...
export function ProfileSteam(props) {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                const result = await fetch(`${process.env.REACT_APP_URLBackend}/steam/findGetPlayerSummaries/UserId/${props.steamId}/`, {
                    method: 'GET',
                    headers: {
            ...
            try {
                const page = 1;
                const result = await fetch(`${process.env.REACT_APP_URLBackend}/steam/findGetPlayerGames/Userid/${props.steamId}/${page}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        token: authToken
    ...
    async function buscarConquistasSteam(nextPage) {
        ...
        try {
          const result = await fetch(`${process.env.REACT_APP_URLBackend}/steam/findGetPlayerGames/Userid/${props.steamId}/${nextPage}`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              token: authToken
