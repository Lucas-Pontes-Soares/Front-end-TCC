...
export function GetRequestByUserId() {
    ...
    (async () => {
      ...
      try {
        ...
        const resultado = await fetch(
          `${process.env.REACT_APP_URLBackend}/playerrequest/getPlayerRequestById/${userId}`,
          {
            method: 'GET',
            headers: {
  ...
  async function deleteRequest(idRequest) {
    ...
    try {
      const resultado = await fetch(`${process.env.REACT_APP_URLBackend}/playerrequest/deletePlayerRequest/${idRequest}`, {
        method: "DELETE",
        headers: {
          'Content-Type': 'application/json',
          token: localStorage.getItem("AuthToken")
